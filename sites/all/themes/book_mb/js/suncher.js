$(function() {
	...
	function selectItem() {
		...
		function createSelect(select_container, index) {
			...
			//点击显示框
			$('.select_showbox').bind('click',function() {
				ul_option.css({"display":"block"});
	...
	function srcoll(){
		...
		var iNow = 0;
		var timer = null;
		var size = $(".newsList li").size();
		iH = $('.newsList li').height();	
		...
		function doMove( num ) {
			iNow += num;
			if ( Math.abs(iNow) > size-1 ) {
				iNow = 0;
			}
			if ( iNow > 0 ) {
				iNow = -(size-1);
			}
			$('.newsList ul').stop().animate({ 'top': iH*iNow }, 1000);
		}
	}
});
